...
export interface G_ShoppingListLineItem<
  Timestamp,
...
> {
  ...
  cart?: {
    addedAt: Timestamp;
    addedBy: string;
    storeRef: StoreDocumentReference; // reference
    pricePerUnit: number;
  };
  locked?: boolean;
  product: {
...
export type G_ShoppingListStore<Timestamp, StoreDocumentReference, GeoPoint> =
  Omit<G_PartialStore<Timestamp, StoreDocumentReference, GeoPoint>, 'distance'>;

export interface G_ShoppingList<
  Timestamp,
  StoreDocumentReference,
  GeoPoint,
...
> {
  ...
  shoppingListRef: ShoppingListReference; // reference
  uid: string;
}
